...
import { FContext, FlightContext, type FlightWithPlane } from '../providers/flight-provider'
import { getUrlFile } from '@/lib/supabase'
import { CHECKOUT_KEY, dateFormat, rupiahFormat, SEAT_VALUES, SeatValuesType } from '@/lib/utils'
        ...
        <div className="ticket-card flex justify-between items-center rounded-[20px] p-5 bg-flysha-bg-purple">
            <div className="flex gap-[16px] items-center">
                <div className="flex shrink-0 w-[90px] h-[70px] rounded-[14px] overflow-hidden">
                    <Image width={60} height={60} src={getUrlFile(data.plane.image)} className="w-full h-full object-cover" alt="thumbnail" />
                </div>
            ...
            <button type='button' onClick={bookNow} className="font-bold text-flysha-black bg-flysha-light-purple rounded-full p-[12px_20px] h-[48px] transition-all duration-300 hover:shadow-[0_10px_20px_0_#B88DFF]">Book Flight</button>
        </div>
