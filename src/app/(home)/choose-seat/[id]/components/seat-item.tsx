...
export default function SeatItem({seat}: SeatItemProps) {
    ...
    return (
        <div className="group flex shrink-0 w-[60px] h-[60px] items-center justify-center relative">
            ...
            <input type="radio" name="seat" onClick={() => {
                setSelectedSeat(seat)
            }} id={seat.seatNumber} className="w-[60px] h-[60px] appearance-none rounded-[15px] checked:bg-flysha-light-purple ring-2 ring-white checked:ring-flysha-light-purple disabled:ring-0 disabled:bg-flysha-dark-grey" disabled={seat.isBooked ?? false} />
        </div>
    )
}
