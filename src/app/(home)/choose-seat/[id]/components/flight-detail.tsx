...
import { Checkout, dateFormat, rupiahFormat, SEAT_VALUES, SeatValuesType } from '@/lib/utils'
...
export default function FlightDetail({flight, session}: FlightDetailProps) {
    ...
    const continueBook = () => {
        if (seat === null) {
            toast("Failed to checkout, select a seat first")
            return
        }

        if (session === null) {
            toast("Failed to checkout, please login first")
    ...
    return (
        <div className="flex flex-col items-center gap-[30px] mt-[61px] pb-[30px]">
            <h1 className="font-bold text-[32px] leading-[48px] text-center">{flight.departureCity} to {flight.destinationCity}</h1>
            <div className="flex flex-col items-center gap-[30px] w-[335px]">
                ...
                <div className="flex flex-col gap-4 w-full">
                    <div className="flex shrink-0 w-full h-[130px] rounded-[14px] overflow-hidden">
                        <img src={getUrlFile(flight.plane.image)} className="w-full h-full object-cover" alt="image" />
                    </div>
                    <div className="flex justify-between items-center">
                        <div className="flex flex-col gap-[2px]">
                            <p className="font-bold text-lg">{flight.plane.name}</p>
                            <p className="text-sm text-flysha-grey">{flight.plane.code} • {selectedSeat.label}</p>
                        </div>
                ...
                <button type='button' onClick={continueBook} className="font-bold text-flysha-black bg-flysha-light-purple rounded-full h-12 w-full transition-all duration-300 hover:shadow-[0_10px_20px_0_#B88DFF] flex justify-center items-center">Continue to Book</button>
            </div>
        </div>
    )
}
